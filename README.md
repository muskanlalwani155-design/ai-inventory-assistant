...
* **AI & LLM Engine**: Google Gemini API (`gemini-3.5-flash-lite`, Text-to-SQL & summarization)
* **Database & Cloud Storage**: TiDB Cloud (Serverless MySQL 8.0 compatible)
* **Transaction Engine**: MySQL connector with support for ACID transactions
...
* **Google Gemini API (Text-to-SQL)**: Converts natural language requests into schema-aware, optimized SQL queries using `gemini-3.5-flash-lite`.
...
* **Order Processing & Transactions**: Handles ACID-compliant state changes and order management via `order_service.py`.
* **TiDB / Cloud MySQL Database**: Executes verified queries against structured inventory and sales tables (`schema.sql`).
...
* **ACID Transactions**: Reliable database execution for order processing and inventory updates.
* **Query Safety Guardrails**: Restricts ad-hoc execution strictly to read-only (`SELECT`) queries.
* **Automated Cloud DB Migration**: Dedicated setup scripts to bootstrap remote schemas seamlessly (`setup_cloud_db.py`).
* **Dual Execution Modes**: Interactive web dashboard via Streamlit (`ui_app.py`) as well as CLI/terminal mode (`app.py`).
